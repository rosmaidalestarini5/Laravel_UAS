...
class JualController extends Controller
{
    ...
    public function addtocart($id)
    {
        try{
            DB::transaction(function () use ($id){
                ...
                $jual_detail = JualDetail::whereRaw("jual_id=? AND pizza_id=?",[$jual->id, $pizza->id])->first();
                    if($jual_detail == null){
                        ...
                        $jual_detail->sub_total = $jual_detail->qty * $jual_detail->harga_satuan;
                        $jual_detail->save();
                    }else{
                        $jual_detail->qty++;
                        $jual_detail->sub_total = $jual_detail->qty * $jual_detail->harga_satuan;
    ...
    public function getcart()
    {
        ...
        if($jual==null) return redirect('/konsumen/order')->withErrors(
            ...
            if(count($jual_details)==0) return redirect('/kosumen/order')->withErrors(
                ['msg' => 'Cart masih kosong']);
    ...
    public function postcart(Request $request)
    {
        ...
        try{
            DB::transaction(function () use ($request, &$pesan, &$redirect_url){
                if($request->action_button=='checkout' || $request->action_button=='order'){
    ...
    public function postcheckout(Request $request)
    {
        ...
        //        return redirect('/konsumen/checkout')->with('success', 'Pesanan berhasil 
        //        dikirim!');
    ...
    public function gettrack(Request $request, $id)
    {
        ...
        $alamat_kirim = AlamatKirim::find($jual->alamat_kirim_id)->first();
        ...
        return view('konsumen.jual.track', compact('jual', 'jual_details','alamat_kirim','kurir'));
    }
